...
public class FaixasValoresLicencaMaternidadeService {

	@Autowired
	private FaixasValoresLicencaMaternidadeReposytory reposytory;
	
	@Autowired
	private AnoMesService anoMesService;
	

	public void salvar(FaixasValoresLicencaMaternidade faixasValoresLicencaMaternidade) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresLicencaMaternidade);
	}

	public void editar(FaixasValoresLicencaMaternidade faixasValoresLicencaMaternidade) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresLicencaMaternidade);
	}
	...
	public FaixasValoresLicencaMaternidade buscarPorId(Long id) {
		// TODO Auto-generated method stub
		return reposytory.findById(id).get();
	}

	@Transactional(readOnly = true)
	public List<FaixasValoresLicencaMaternidade> buscarTodos() {
		// TODO Auto-generated method stub
		return reposytory.findByDtCancelamentoIsNullOrderByIdFuncionarioFkIdPessoaFkNomeAscDtInicialAsc();
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresLicencaMaternidade> buscarPorFuncionarioExato(PessoaFuncionarios funcionario) {
		return reposytory.findByIdFuncionarioFkAndDtCancelamentoIsNullOrderByIdFuncionarioFkIdPessoaFkNomeAscDtInicialAsc(funcionario);
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresLicencaMaternidade> buscarPorNome(String nome) {
		return reposytory.findByIdFuncionarioFkIdPessoaFkNomeContainingAndDtCancelamentoIsNullOrderByIdFuncionarioFkIdPessoaFkNomeAscDtInicialAsc(nome);
	}
	
	public Page<FaixasValoresLicencaMaternidade> findPaginated(int pageNo, int pageSize) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
		return this.reposytory.findByDtCancelamentoIsNullOrderByIdFuncionarioFkIdPessoaFkNomeAscDtInicialAsc(pageable);
	}

	public Page<FaixasValoresLicencaMaternidade> findPaginatedNome(int pageNo, int pageSize, String nome) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
		return this.reposytory.findByIdFuncionarioFkIdPessoaFkNomeContainingAndDtCancelamentoIsNullOrderByIdFuncionarioFkIdPessoaFkNomeAscDtInicialAsc(nome.toUpperCase().trim(), pageable);
	}
	

	
	
	public ByteArrayInputStream exportarExcel(List<FaixasValoresLicencaMaternidade> lista) {
		try(Workbook workbook = new XSSFWorkbook()){
			Sheet sheet = workbook.createSheet("Licenca Maternidade");
	        ...
	        cell.setCellValue("Dt Inicio");
	        cell.setCellStyle(headerCellStyle);
	        
	        cell = row.createCell(5);
	        cell.setCellValue("Dt Fim");
	        cell.setCellStyle(headerCellStyle);
	        ...
	        for(int i = 0; i < lista.size(); i++) {
	        	...
	        	dataRow.createCell(2).setCellValue(lista.get(i).getIdFuncionarioFk().getIdPessoaFk().getNome());
	        	dataRow.createCell(3).setCellValue(lista.get(i).getIdFuncionarioFk().getIdPessoaFk().getCpf());
	        	dataRow.createCell(4).setCellValue(lista.get(i).getDtInicial());
	...
	public ByteArrayInputStream exportarPdf(List<FaixasValoresLicencaMaternidade> lista) {
		...
		try {
			...
			hcell = new PdfPCell(new Phrase("Dt Inicio", cabecalhoFont));
			hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(hcell);
			
			hcell = new PdfPCell(new Phrase("Dt Fim", cabecalhoFont));
			hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(hcell);
			...
			hcell = new PdfPCell(new Phrase("Valor Bruto por Dia", cabecalhoFont));
			hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(hcell);
			...
			for (int i=0; i<lista.size();i++) {
				...
				cell = new PdfPCell(new Phrase(String.valueOf(lista.get(i).getValorBrutoPorDia()) ,corpoFont) );
				cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				cell.setHorizontalAlignment(Element.ALIGN_CENTER);
				table.addCell(cell);
			...
			PdfPCell cellTitulo;
			cellTitulo = new PdfPCell(new Phrase("Faixas Valores Licença Maternidade", tituloFont) );
