...
import com.folha.boot.Reposytory.FaixasValoresGpfCedidoReposytory;
...
public class FaixasValoresGpfCedidoService {

	@Autowired
	private FaixasValoresGpfCedidoReposytory reposytory;
	
	@Autowired
	private AnoMesService anoMesService;
	

	public void salvar(FaixasValoresGpfCedido faixasValoresGpfCedido) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresGpfCedido);
	}

	public void editar(FaixasValoresGpfCedido faixasValoresGpfCedido) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresGpfCedido);
	}
	...
	public FaixasValoresGpfCedido buscarPorId(Long id) {
		// TODO Auto-generated method stub
		return reposytory.findById(id).get();
	}

	@Transactional(readOnly = true)
	public List<FaixasValoresGpfCedido> buscarTodos() {
		// TODO Auto-generated method stub
		return reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc();
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresGpfCedido> buscarPorMesExato(AnoMes anoMes) {
		return reposytory.findByIdAnoMesFkOrderByIdAnoMesFkNomeAnoMesDesc(anoMes);
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresGpfCedido> buscarPorNome(String nome) {
		return reposytory.findByIdAnoMesFkNomeAnoMesContainingOrderByIdAnoMesFkNomeAnoMesDesc(nome);
	}
	
	public Page<FaixasValoresGpfCedido> findPaginated(int pageNo, int pageSize) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
		return this.reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc(pageable);
	}

	public Page<FaixasValoresGpfCedido> findPaginatedAnoMes(int pageNo, int pageSize, String nome) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
	...
	public void herdarDeUmMesParaOOutro(Long anoMesInicial, Long anoMesFinal) {
		...
		List<FaixasValoresGpfCedido> listaInicial = buscarPorMesExato(anoMesService.buscarPorId(anoMesInicial)); 
		List<FaixasValoresGpfCedido> listaFinal = buscarPorMesExato(anoMesService.buscarPorId(anoMesFinal));
		
		if( (!listaInicial.isEmpty())  &&  (listaFinal.isEmpty()) ) {
			for(int i=0;i<listaInicial.size();i++) {
				FaixasValoresGpfCedido f = new FaixasValoresGpfCedido();
				f.setId(null);
	...
	public ByteArrayInputStream exportarExcel(List<FaixasValoresGpfCedido> lista) {
		try(Workbook workbook = new XSSFWorkbook()){
	        ...
	        for(int i = 0; i < lista.size(); i++) {
	        	...
	        	dataRow.createCell(4).setCellValue(lista.get(i).getIdVinculoFk().getNomeVinculo());
	        	dataRow.createCell(5).setCellValue(lista.get(i).getIdUnidadeFk().getNomeFantasia());
	        	dataRow.createCell(6).setCellValue(lista.get(i).getIdFonteFk().getNome());
	        	dataRow.createCell(7).setCellValue(lista.get(i).getIdTipoBrutoLiquidoFk().getDescricao());
	...
	public ByteArrayInputStream exportarPdf(List<FaixasValoresGpfCedido> lista) {
		...
		try {
			...
			hcell = new PdfPCell(new Phrase("Vínculo", cabecalhoFont));
			hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(hcell);
			...
			for (int i=0; i<lista.size();i++) {
				...
				cell = new PdfPCell(new Phrase(lista.get(i).getIdVinculoFk().getNomeVinculo() ,corpoFont) );
				cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				cell.setHorizontalAlignment(Element.ALIGN_CENTER);
				table.addCell(cell);
			...
			PdfPCell cellTitulo;
			cellTitulo = new PdfPCell(new Phrase("Faixas Valores Gpf Cedido", tituloFont) );
