...
import com.folha.boot.Reposytory.FaixasValoresGpfMedicaReposytory;
...
public class FaixasValoresGpfMedicaService {

	@Autowired
	private FaixasValoresGpfMedicaReposytory reposytory;
	
	@Autowired
	private AnoMesService anoMesService;
	

	public void salvar(FaixasValoresGpfMedica faixasValoresGpfMedica) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresGpfMedica);
	}

	public void editar(FaixasValoresGpfMedica faixasValoresGpfMedica) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresGpfMedica);
	}
	...
	public FaixasValoresGpfMedica buscarPorId(Long id) {
		// TODO Auto-generated method stub
		return reposytory.findById(id).get();
	}

	@Transactional(readOnly = true)
	public List<FaixasValoresGpfMedica> buscarTodos() {
		// TODO Auto-generated method stub
		return reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc();
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresGpfMedica> buscarPorMesExato(AnoMes anoMes) {
		return reposytory.findByIdAnoMesFkOrderByIdAnoMesFkNomeAnoMesDesc(anoMes);
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresGpfMedica> buscarPorNome(String nome) {
		return reposytory.findByIdAnoMesFkNomeAnoMesContainingOrderByIdAnoMesFkNomeAnoMesDesc(nome);
	}
	
	public Page<FaixasValoresGpfMedica> findPaginated(int pageNo, int pageSize) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
		return this.reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc(pageable);
	}

	public Page<FaixasValoresGpfMedica> findPaginatedAnoMes(int pageNo, int pageSize, String nome) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
	...
	public void herdarDeUmMesParaOOutro(Long anoMesInicial, Long anoMesFinal) {
		...
		List<FaixasValoresGpfMedica> listaInicial = buscarPorMesExato(anoMesService.buscarPorId(anoMesInicial)); 
		List<FaixasValoresGpfMedica> listaFinal = buscarPorMesExato(anoMesService.buscarPorId(anoMesFinal));
		
		if( (!listaInicial.isEmpty())  &&  (listaFinal.isEmpty()) ) {
			for(int i=0;i<listaInicial.size();i++) {
				FaixasValoresGpfMedica f = new FaixasValoresGpfMedica();
				f.setId(null);
				...
				f.setValorFimSemana(listaInicial.get(i).getValorFimSemana());
				f.setValorSemana(listaInicial.get(i).getValorSemana());
	...
	public ByteArrayInputStream exportarExcel(List<FaixasValoresGpfMedica> lista) {
		try(Workbook workbook = new XSSFWorkbook()){
	        ...
	        cell.setCellValue("Valor Semana");
	        cell.setCellStyle(headerCellStyle);
	        
	        cell = row.createCell(10);
	        cell.setCellValue("Valor Fim Semana");
	        cell.setCellStyle(headerCellStyle);
	        ...
	        for(int i = 0; i < lista.size(); i++) {
	        	...
	        	dataRow.createCell(9).setCellValue(lista.get(i).getValorSemana());
	        	dataRow.createCell(10).setCellValue(lista.get(i).getValorFimSemana());
	...
	public ByteArrayInputStream exportarPdf(List<FaixasValoresGpfMedica> lista) {
		...
		try {
			...
			hcell = new PdfPCell(new Phrase("Valor Sem", cabecalhoFont));
			hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(hcell);
			
			hcell = new PdfPCell(new Phrase("Valor F Sem", cabecalhoFont));
			hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(hcell);
			...
			for (int i=0; i<lista.size();i++) {
				...
				cell = new PdfPCell(new Phrase(String.valueOf(lista.get(i).getValorSemana()) ,corpoFont) );
				cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				cell.setHorizontalAlignment(Element.ALIGN_CENTER);
				table.addCell(cell);
				
				cell = new PdfPCell(new Phrase(String.valueOf(lista.get(i).getValorFimSemana()) ,corpoFont) );
				cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				cell.setHorizontalAlignment(Element.ALIGN_CENTER);
				table.addCell(cell);
			...
			PdfPCell cellTitulo;
			cellTitulo = new PdfPCell(new Phrase("Faixas Valores Gpf Medica", tituloFont) );
