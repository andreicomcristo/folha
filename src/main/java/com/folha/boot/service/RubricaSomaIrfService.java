...
import com.folha.boot.Reposytory.RubricaSomaIrfReposytory;
...
public class RubricaSomaIrfService {

	@Autowired
	private RubricaSomaIrfReposytory reposytory;
	
	@Autowired
	private AnoMesService anoMesService;
	

	public void salvar(RubricaSomaIrf rubricaSomaIrf) {
		// TODO Auto-generated method stub
		reposytory.save(rubricaSomaIrf);
	}

	public void editar(RubricaSomaIrf rubricaSomaIrf) {
		// TODO Auto-generated method stub
		reposytory.save(rubricaSomaIrf);
	}
	...
	public RubricaSomaIrf buscarPorId(Long id) {
		// TODO Auto-generated method stub
		return reposytory.findById(id).get();
	}

	@Transactional(readOnly = true)
	public List<RubricaSomaIrf> buscarTodos() {
		// TODO Auto-generated method stub
		return reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc();
	}
	
	@Transactional(readOnly = true)
	public List<RubricaSomaIrf> buscarPorMesExato(AnoMes anoMes) {
		return reposytory.findByIdAnoMesFkOrderByIdAnoMesFkNomeAnoMesDesc(anoMes);
	}
	
	@Transactional(readOnly = true)
	public List<RubricaSomaIrf> buscarPorNome(String nome) {
		return reposytory.findByIdAnoMesFkNomeAnoMesContainingOrderByIdAnoMesFkNomeAnoMesDesc(nome);
	}
	
	public Page<RubricaSomaIrf> findPaginated(int pageNo, int pageSize) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
		return this.reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc(pageable);
	}

	public Page<RubricaSomaIrf> findPaginatedAnoMes(int pageNo, int pageSize, String nome) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
	...
	public boolean avaliarCadastrado( RubricaSomaIrfCodigo rubricaSomaIrfCodigo, AnoMes anoMes) {
		boolean resposta = false;
		List<RubricaSomaIrf> lista = reposytory.findByIdCodigoFkAndIdAnoMesFk( rubricaSomaIrfCodigo, anoMes); 
	...
	public void herdarDeUmMesParaOOutro(Long anoMesInicial, Long anoMesFinal) {
		...
		List<RubricaSomaIrf> listaInicial = buscarPorMesExato(anoMesService.buscarPorId(anoMesInicial)); 
		List<RubricaSomaIrf> listaFinal = buscarPorMesExato(anoMesService.buscarPorId(anoMesFinal));
		
		if( (!listaInicial.isEmpty())  &&  (listaFinal.isEmpty()) ) {
			for(int i=0;i<listaInicial.size();i++) {
				RubricaSomaIrf f = new RubricaSomaIrf();
				f.setId(null);
	...
	public ByteArrayInputStream exportarExcel(List<RubricaSomaIrf> lista) {
	...
	public ByteArrayInputStream exportarPdf(List<RubricaSomaIrf> lista) {
		...
		try {
			...
			PdfPCell cellTitulo;
			cellTitulo = new PdfPCell(new Phrase("Rubrica Soma IRF", tituloFont) );
