...
import com.folha.boot.Reposytory.SalarioMinimoReposytory;
...
public class SalarioMinimoService {
	...
	@Autowired
	private SalarioMinimoReposytory reposytory;
	...
	public void salvar(SalarioMinimo salarioMinimo) {
		...
		reposytory.save(salarioMinimo);
	}

	public void editar(SalarioMinimo salarioMinimo) {
		// TODO Auto-generated method stub
		reposytory.save(salarioMinimo);
	}
	...
	public SalarioMinimo buscarPorId(Long id) {
		// TODO Auto-generated method stub
		return reposytory.findById(id).get();
	}

	@Transactional(readOnly = true)
	public List<SalarioMinimo> buscarTodos() {
		// TODO Auto-generated method stub
		return reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc();
	}
	
	@Transactional(readOnly = true)
	public List<SalarioMinimo> buscarPorMesExato(AnoMes anoMes) {
		return reposytory.findByIdAnoMesFkOrderByIdAnoMesFkNomeAnoMesDesc(anoMes);
	}
	
	@Transactional(readOnly = true)
	public List<SalarioMinimo> buscarPorNome(String nome) {
		return reposytory.findByIdAnoMesFkNomeAnoMesContainingOrderByIdAnoMesFkNomeAnoMesDesc(nome);
	}
	
	public Page<SalarioMinimo> findPaginated(int pageNo, int pageSize) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
		return this.reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc(pageable);
	}

	public Page<SalarioMinimo> findPaginatedAnoMes(int pageNo, int pageSize, String nome) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
	...
	public void herdarDeUmMesParaOOutro(Long anoMesInicial, Long anoMesFinal) {
		...
		List<SalarioMinimo> listaInicial = buscarPorMesExato(anoMesService.buscarPorId(anoMesInicial)); 
		List<SalarioMinimo> listaFinal = buscarPorMesExato(anoMesService.buscarPorId(anoMesFinal));
		
		if( (!listaInicial.isEmpty())  &&  (listaFinal.isEmpty()) ) {
			for(int i=0;i<listaInicial.size();i++) {
				SalarioMinimo f = new SalarioMinimo();
				f.setId(null);
	...
	public ByteArrayInputStream exportarExcel(List<SalarioMinimo> lista) {
		try(Workbook workbook = new XSSFWorkbook()){
	        ...
	        for(int i = 0; i < lista.size(); i++) {
	        	...
	        	dataRow.createCell(3).setCellValue(lista.get(i).getValor() );
	...
	public ByteArrayInputStream exportarPdf(List<SalarioMinimo> lista) {
		...
		try {
			...
			PdfPCell cellTitulo;
			cellTitulo = new PdfPCell(new Phrase("Salario Minimo", tituloFont) );
