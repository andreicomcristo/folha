...
import com.folha.boot.Reposytory.FaixasValoresGpfReposytory;
...
public class FaixasValoresGpfService {

	@Autowired
	private FaixasValoresGpfReposytory reposytory;
	
	@Autowired
	private AnoMesService anoMesService;
	

	public void salvar(FaixasValoresGpf faixasValoresGpf) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresGpf);
	}

	public void editar(FaixasValoresGpf faixasValoresGpf) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresGpf);
	}
	...
	public FaixasValoresGpf buscarPorId(Long id) {
		// TODO Auto-generated method stub
		return reposytory.findById(id).get();
	}

	@Transactional(readOnly = true)
	public List<FaixasValoresGpf> buscarTodos() {
		// TODO Auto-generated method stub
		return reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc();
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresGpf> buscarPorMesExato(AnoMes anoMes) {
		return reposytory.findByIdAnoMesFkOrderByIdAnoMesFkNomeAnoMesDesc(anoMes);
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresGpf> buscarPorNome(String nome) {
		return reposytory.findByIdAnoMesFkNomeAnoMesContainingOrderByIdAnoMesFkNomeAnoMesDesc(nome);
	}
	
	public Page<FaixasValoresGpf> findPaginated(int pageNo, int pageSize) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
		return this.reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc(pageable);
	}

	public Page<FaixasValoresGpf> findPaginatedAnoMes(int pageNo, int pageSize, String nome) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
	...
	public void herdarDeUmMesParaOOutro(Long anoMesInicial, Long anoMesFinal) {
		...
		List<FaixasValoresGpf> listaInicial = buscarPorMesExato(anoMesService.buscarPorId(anoMesInicial)); 
		List<FaixasValoresGpf> listaFinal = buscarPorMesExato(anoMesService.buscarPorId(anoMesFinal));
		
		if( (!listaInicial.isEmpty())  &&  (listaFinal.isEmpty()) ) {
			for(int i=0;i<listaInicial.size();i++) {
				FaixasValoresGpf f = new FaixasValoresGpf();
				f.setId(null);
	...
	public ByteArrayInputStream exportarExcel(List<FaixasValoresGpf> lista) {
	...
	public ByteArrayInputStream exportarPdf(List<FaixasValoresGpf> lista) {
		...
		try {
			...
			PdfPCell cellTitulo;
			cellTitulo = new PdfPCell(new Phrase("Faixas Valores Gpf", tituloFont) );
