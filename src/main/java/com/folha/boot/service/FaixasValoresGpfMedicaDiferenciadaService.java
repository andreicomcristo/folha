...
import com.folha.boot.Reposytory.FaixasValoresGpfMedicaDiferenciadaReposytory;
...
public class FaixasValoresGpfMedicaDiferenciadaService {

	@Autowired
	private FaixasValoresGpfMedicaDiferenciadaReposytory reposytory;
	
	@Autowired
	private AnoMesService anoMesService;
	

	public void salvar(FaixasValoresGpfMedicaDiferenciada faixasValoresGpfMedicaDiferenciada) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresGpfMedicaDiferenciada);
	}

	public void editar(FaixasValoresGpfMedicaDiferenciada faixasValoresGpfMedicaDiferenciada) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresGpfMedicaDiferenciada);
	}
	...
	public FaixasValoresGpfMedicaDiferenciada buscarPorId(Long id) {
		// TODO Auto-generated method stub
		return reposytory.findById(id).get();
	}

	@Transactional(readOnly = true)
	public List<FaixasValoresGpfMedicaDiferenciada> buscarTodos() {
		// TODO Auto-generated method stub
		return reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc();
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresGpfMedicaDiferenciada> buscarPorMesExato(AnoMes anoMes) {
		return reposytory.findByIdAnoMesFkOrderByIdAnoMesFkNomeAnoMesDesc(anoMes);
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresGpfMedicaDiferenciada> buscarPorNome(String nome) {
		return reposytory.findByIdAnoMesFkNomeAnoMesContainingOrderByIdAnoMesFkNomeAnoMesDesc(nome);
	}
	
	public Page<FaixasValoresGpfMedicaDiferenciada> findPaginated(int pageNo, int pageSize) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
		return this.reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc(pageable);
	}

	public Page<FaixasValoresGpfMedicaDiferenciada> findPaginatedAnoMes(int pageNo, int pageSize, String nome) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
	...
	public void herdarDeUmMesParaOOutro(Long anoMesInicial, Long anoMesFinal) {
		...
		List<FaixasValoresGpfMedicaDiferenciada> listaInicial = buscarPorMesExato(anoMesService.buscarPorId(anoMesInicial)); 
		List<FaixasValoresGpfMedicaDiferenciada> listaFinal = buscarPorMesExato(anoMesService.buscarPorId(anoMesFinal));
		
		if( (!listaInicial.isEmpty())  &&  (listaFinal.isEmpty()) ) {
			for(int i=0;i<listaInicial.size();i++) {
				FaixasValoresGpfMedicaDiferenciada f = new FaixasValoresGpfMedicaDiferenciada();
				f.setId(null);
	...
	public ByteArrayInputStream exportarExcel(List<FaixasValoresGpfMedicaDiferenciada> lista) {
	...
	public ByteArrayInputStream exportarPdf(List<FaixasValoresGpfMedicaDiferenciada> lista) {
		...
		try {
			...
			PdfPCell cellTitulo;
			cellTitulo = new PdfPCell(new Phrase("Faixas Valores Gpf Medica Diferenciada", tituloFont) );
