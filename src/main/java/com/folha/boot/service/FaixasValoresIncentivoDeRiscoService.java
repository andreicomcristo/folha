...
import com.folha.boot.Reposytory.FaixasValoresIncentivoDeRiscoReposytory;
...
public class FaixasValoresIncentivoDeRiscoService {

	@Autowired
	private FaixasValoresIncentivoDeRiscoReposytory reposytory;
	
	@Autowired
	private AnoMesService anoMesService;
	

	public void salvar(FaixasValoresIncentivoDeRisco faixasValoresincentivoDeRisco) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresincentivoDeRisco);
	}

	public void editar(FaixasValoresIncentivoDeRisco faixasValoresincentivoDeRisco) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresincentivoDeRisco);
	}
	...
	public FaixasValoresIncentivoDeRisco buscarPorId(Long id) {
		// TODO Auto-generated method stub
		return reposytory.findById(id).get();
	}

	@Transactional(readOnly = true)
	public List<FaixasValoresIncentivoDeRisco> buscarTodos() {
		// TODO Auto-generated method stub
		return reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc();
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresIncentivoDeRisco> buscarPorMesExato(AnoMes anoMes) {
		return reposytory.findByIdAnoMesFkOrderByIdAnoMesFkNomeAnoMesDesc(anoMes);
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresIncentivoDeRisco> buscarPorNome(String nome) {
		return reposytory.findByIdAnoMesFkNomeAnoMesContainingOrderByIdAnoMesFkNomeAnoMesDesc(nome);
	}
	
	public Page<FaixasValoresIncentivoDeRisco> findPaginated(int pageNo, int pageSize) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
		return this.reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc(pageable);
	}

	public Page<FaixasValoresIncentivoDeRisco> findPaginatedAnoMes(int pageNo, int pageSize, String nome) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
	...
	public void herdarDeUmMesParaOOutro(Long anoMesInicial, Long anoMesFinal) {
		...
		List<FaixasValoresIncentivoDeRisco> listaInicial = buscarPorMesExato(anoMesService.buscarPorId(anoMesInicial)); 
		List<FaixasValoresIncentivoDeRisco> listaFinal = buscarPorMesExato(anoMesService.buscarPorId(anoMesFinal));
		
		if( (!listaInicial.isEmpty())  &&  (listaFinal.isEmpty()) ) {
			for(int i=0;i<listaInicial.size();i++) {
				FaixasValoresIncentivoDeRisco f = new FaixasValoresIncentivoDeRisco();
				f.setId(null);
	...
	public ByteArrayInputStream exportarExcel(List<FaixasValoresIncentivoDeRisco> lista) {
	...
	public ByteArrayInputStream exportarPdf(List<FaixasValoresIncentivoDeRisco> lista) {
		...
		try {
			...
			PdfPCell cellTitulo;
			cellTitulo = new PdfPCell(new Phrase("Faixas Valores Incentivo de Risco", tituloFont) );
