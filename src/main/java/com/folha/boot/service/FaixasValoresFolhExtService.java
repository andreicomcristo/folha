...
import com.folha.boot.Reposytory.FaixasValoresFolhExtReposytory;
...
public class FaixasValoresFolhExtService {

	@Autowired
	private FaixasValoresFolhExtReposytory reposytory;
	
	@Autowired
	private AnoMesService anoMesService;
	

	public void salvar(FaixasValoresFolhExt faixasValoresFolhExt) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresFolhExt);
	}

	public void editar(FaixasValoresFolhExt faixasValoresFolhExt) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresFolhExt);
	}
	...
	public FaixasValoresFolhExt buscarPorId(Long id) {
		// TODO Auto-generated method stub
		return reposytory.findById(id).get();
	}

	@Transactional(readOnly = true)
	public List<FaixasValoresFolhExt> buscarTodos() {
		// TODO Auto-generated method stub
		return reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc();
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresFolhExt> buscarPorMesExato(AnoMes anoMes) {
		return reposytory.findByIdAnoMesFkOrderByIdAnoMesFkNomeAnoMesDesc(anoMes);
	}
	
	@Transactional(readOnly = true)
	public List<FaixasValoresFolhExt> buscarPorNome(String nome) {
		return reposytory.findByIdAnoMesFkNomeAnoMesContainingOrderByIdAnoMesFkNomeAnoMesDesc(nome);
	}
	
	public Page<FaixasValoresFolhExt> findPaginated(int pageNo, int pageSize) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
		return this.reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDesc(pageable);
	}

	public Page<FaixasValoresFolhExt> findPaginatedAnoMes(int pageNo, int pageSize, String nome) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
	...
	public void herdarDeUmMesParaOOutro(Long anoMesInicial, Long anoMesFinal) {
		...
		List<FaixasValoresFolhExt> listaInicial = buscarPorMesExato(anoMesService.buscarPorId(anoMesInicial)); 
		List<FaixasValoresFolhExt> listaFinal = buscarPorMesExato(anoMesService.buscarPorId(anoMesFinal));
		
		if( (!listaInicial.isEmpty())  &&  (listaFinal.isEmpty()) ) {
			for(int i=0;i<listaInicial.size();i++) {
				FaixasValoresFolhExt f = new FaixasValoresFolhExt();
				f.setId(null);
	...
	public ByteArrayInputStream exportarExcel(List<FaixasValoresFolhExt> lista) {
		try(Workbook workbook = new XSSFWorkbook()){
	        ...
	        for(int i = 0; i < lista.size(); i++) {
	        	...
	        	dataRow.createCell(5).setCellValue(lista.get(i).getIdNivelCargoFk().getNomeNivelCargo());
	...
	public ByteArrayInputStream exportarPdf(List<FaixasValoresFolhExt> lista) {
		...
		try {
			...
			PdfPCell cellTitulo;
			cellTitulo = new PdfPCell(new Phrase("Faixas Valores Folh Ext", tituloFont) );
