...
import com.folha.boot.Reposytory.FaixasValoresParametrosCalculoFolhasExtrasIndividualReposytory;
...
public class FaixasValoresParametrosCalculoFolhasExtrasIndividualService {

	@Autowired
	private FaixasValoresParametrosCalculoFolhasExtrasIndividualReposytory reposytory;
	
	@Autowired
	private AnoMesService anoMesService;
	

	public void salvar(FaixasValoresParametrosCalculoFolhasExtrasIndividual faixasValoresParametrosCalculoFolhasExtrasIndividual) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresParametrosCalculoFolhasExtrasIndividual);
	}

	public void editar(FaixasValoresParametrosCalculoFolhasExtrasIndividual faixasValoresParametrosCalculoFolhasExtrasIndividual) {
		// TODO Auto-generated method stub
		reposytory.save(faixasValoresParametrosCalculoFolhasExtrasIndividual);
	}
	...
	public FaixasValoresParametrosCalculoFolhasExtrasIndividual buscarPorId(Long id) {
		// TODO Auto-generated method stub
		return reposytory.findById(id).get();
	}

	@Transactional(readOnly = true)
	public List<FaixasValoresParametrosCalculoFolhasExtrasIndividual> buscarTodos() {
		// TODO Auto-generated method stub
		return reposytory.findAllByOrderByIdAnoMesFkNomeAnoMesDescIdCodDiferenciadoFkIdUnidadeFkNomeFantasiaAsc();
	}
	@Transactional(readOnly = true)
	public List<FaixasValoresParametrosCalculoFolhasExtrasIndividual> buscarPorNome(String nome) {
	...
	@Transactional(readOnly = true)
	public List<FaixasValoresParametrosCalculoFolhasExtrasIndividual> buscarPorMesExato(AnoMes anoMes) {
		return reposytory.findByIdAnoMesFkOrderByIdAnoMesFkNomeAnoMesDescIdCodDiferenciadoFkIdUnidadeFkNomeFantasiaAsc(anoMes);
	}
	
	
	@Transactional(readOnly = true)
	public List<FaixasValoresParametrosCalculoFolhasExtrasIndividual> buscarPorMesExatoNivelRegimeFolhaUnidade(Escala escala) {
	...
	public Page<FaixasValoresParametrosCalculoFolhasExtrasIndividual> findPaginated(int pageNo, int pageSize) {
	...
	public Page<FaixasValoresParametrosCalculoFolhasExtrasIndividual> findPaginatedAnoMes(int pageNo, int pageSize, String nomeCidade) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
	...
	public Page<FaixasValoresParametrosCalculoFolhasExtrasIndividual> findPaginatedNome(int pageNo, int pageSize, String nome) {
		Pageable pageable = PageRequest.of(pageNo -1, pageSize);
		return this.reposytory.findByIdFuncionarioFkIdPessoaFkNomeContainingOrderByIdAnoMesFkNomeAnoMesDescIdCodDiferenciadoFkIdUnidadeFkNomeFantasiaAsc(nome.toUpperCase().trim(), pageable);
	}
	
	//Herdar de um mes para o outro
	public void herdarDeUmMesParaOOutro(Long anoMesInicial, Long anoMesFinal) {
		
		List<FaixasValoresParametrosCalculoFolhasExtrasIndividual> listaInicial = buscarPorMesExato(anoMesService.buscarPorId(anoMesInicial)); 
		List<FaixasValoresParametrosCalculoFolhasExtrasIndividual> listaFinal = buscarPorMesExato(anoMesService.buscarPorId(anoMesFinal));
		
		if( (!listaInicial.isEmpty())  &&  (listaFinal.isEmpty()) ) {
			for(int i=0;i<listaInicial.size();i++) {
				FaixasValoresParametrosCalculoFolhasExtrasIndividual f = new FaixasValoresParametrosCalculoFolhasExtrasIndividual();
				f.setId(null);
	...
	public ByteArrayInputStream exportarExcel(List<FaixasValoresParametrosCalculoFolhasExtrasIndividual> lista) {
		try(Workbook workbook = new XSSFWorkbook()){
	        ...
	        for(int i = 0; i < lista.size(); i++) {
	        	...
	        	dataRow.createCell(3).setCellValue(lista.get(i).getIdFuncionarioFk().getIdPessoaFk().getNome() +" | "+ lista.get(i).getIdFuncionarioFk().getIdPessoaFk().getCpf() +" | "+lista.get(i).getIdFuncionarioFk().getMatricula());
	        	dataRow.createCell(4).setCellValue(lista.get(i).getIdTipoDeFolhaFk().getNomeTipoFolha());
	        	dataRow.createCell(5).setCellValue(lista.get(i).getValorHoraDia());
	        	dataRow.createCell(6).setCellValue(lista.get(i).getValorHoraNoite());
	        	
	        	dataRow.createCell(7).setCellValue(lista.get(i).getValorHoraA());
	        	dataRow.createCell(8).setCellValue(lista.get(i).getValorHoraB());
	        	dataRow.createCell(9).setCellValue(lista.get(i).getValorHoraC());
	        	
	        	dataRow.createCell(10).setCellValue(lista.get(i).getValorHoraSemana());
	        	dataRow.createCell(11).setCellValue(lista.get(i).getValorHoraFimDeSemana());
	        	dataRow.createCell(12).setCellValue(lista.get(i).getValorLiquidoPorHora());
	        	dataRow.createCell(13).setCellValue(lista.get(i).getValorBrutoPorHora());
	        	dataRow.createCell(14).setCellValue(lista.get(i).getValorBrutoFixoTotal());
	        	dataRow.createCell(15).setCellValue(lista.get(i).getIdNivelFk().getNomeNivelCargo());
	        	dataRow.createCell(16).setCellValue(lista.get(i).getIdRegimeDeTrabalhoFk().getNomeRegimeDeTrabalho());
	        	dataRow.createCell(17).setCellValue(lista.get(i).getIdCodDiferenciadoFk().getNomeCodigoDiferenciado());
	        	dataRow.createCell(18).setCellValue(lista.get(i).getIdCodDiferenciadoFk().getIdUnidadeFk().getCnes());
	        	dataRow.createCell(19).setCellValue(lista.get(i).getIdCodDiferenciadoFk().getIdUnidadeFk().getNomeFantasia());
	        	dataRow.createCell(20).setCellValue(lista.get(i).getObservacao());
	...
	public ByteArrayInputStream exportarPdf(List<FaixasValoresParametrosCalculoFolhasExtrasIndividual> lista) {
		...
		try {
			...
			table.setWidths(new int[] { 2, 2, 2, 9, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 3, 2, 2, 2, 2, 9, 6 });
			...
			hcell = new PdfPCell(new Phrase("Funcionario", cabecalhoFont));
			hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(hcell);
			...
			for (int i=0; i<lista.size();i++) {
				...
				cell = new PdfPCell(new Phrase(lista.get(i).getIdFuncionarioFk().getIdPessoaFk().getNome() +" | "+ lista.get(i).getIdFuncionarioFk().getIdPessoaFk().getCpf() +" | "+lista.get(i).getIdFuncionarioFk().getMatricula() ,corpoFont) );
				cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				cell.setHorizontalAlignment(Element.ALIGN_CENTER);
				table.addCell(cell);
			...
			PdfPCell cellTitulo;
			cellTitulo = new PdfPCell(new Phrase("Faixas Valores Referência Extra Individual", tituloFont) );
